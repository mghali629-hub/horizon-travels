...
export default function EuropeDestinationsPage() {
  ...
  useEffect(() => {
    fetch('/api/tours')
      ...
      .then((data) => {
        if (data.success) {
          const europeTours = data.tours.filter((t: Tour) => t.region.toLowerCase().includes('europe'));
          setTours(europeTours);
        }
      })
    ...
    <div className="min-h-screen bg-[#070B19] text-slate-100 font-sans flex flex-col justify-between">
      ...
      <main className="max-w-6xl mx-auto px-4 py-16 flex-1 space-y-10">
        <div className="text-center space-y-2">
          <span className="text-[#D4AF37] font-mono text-xs uppercase tracking-widest font-bold block">Royal European Estates</span>
          <h1 className="font-serif text-4xl font-bold text-white">European Cliffside & Alpine Expeditions</h1>
          <p className="text-slate-400 text-sm max-w-xl mx-auto">
            Amalfi Coast private speedboat transfers, Zermatt glass igloos, and Michelin wine cellars.
          </p>
        </div>
